...
from passman_gen_mod import passGenerator

# Database Code (you can rename your database file to something less obvious)
with sqlite3.connect("passman.db") as db:
    cursor = db.cursor()
...
# Storing password securely in the vault of the password manager application using sha256 hashing algorithm
...
#   Window layout #######################################
    ...
    lbl = Label(second_frame, text="Offline Password Vault")
    lbl.grid(column=2)
    ...
    lbl = Label(second_frame, text="Application")
